...
- **Two Reopt MCP servers exist and 10 of their tool names collide — register one, never both.** Remote connector `https://mcp.reopt.ai` (streamable-http, client-side OAuth) advertises 30 tools: the 10 shared ones plus EAV (6) and CRM (14, incl. 4 feedback/proposal). Local stdio `reopt mcp` advertises 14: the same 10 plus 4 local-only. Declaring both gives the model two tools for one job. Since CLI 0.6.0 the installed package is an Agent Plugins 1.0.0 bundle (`plugin.json` + `mcp.json` declaring the remote server only, plus a `skills/` copy of the CLI's own skills) — a standards-aware client discovers the remote server from `node_modules/@reopt-ai/cli` without manual wiring. Both surfaces now ship `title` + read-only/destructive/idempotent/open-world hints on every tool; a 0.5.0 `reopt mcp` has none.
...
- **CRM writes are `reopt_customer_note_add` plus two proposals** — `reopt_customer_feedback_propose_reply` and `reopt_customer_propose_note` (0.6.0, `customer:write`). A proposal sends nothing and changes no CRM state; it queues a `WorkspaceProposal` for a member to approve in Studio. Check `reopt_customer_feedback_get.pendingProposals` first. There is no create / update / send / delete — creating customers, editing attributes, or sending messages was deliberately left off. Do not simulate one through the CLI or SDK on a model's initiative.
