...
2. **모듈 docs 로 라우팅한다** — 각 `@reopt-ai/*` 패키지가 자기 `dist/docs/` 를 함께 출판합니다. SKILL.md 는 API 표면을 복제하지 않고, "어떤 상황에 어느 docs 파일을 보라"는 라우팅만 제공합니다. 모듈 docs 는 설치된 버전에 핀 고정되지만, 스킬은 그렇지 않습니다.
...
| [`brandapp-sdk-review`](./skills/brandapp-sdk-review/) | 기존 SDK 코드를 감사. 36개 안티패턴을 카테고리(init / Auth / Error / Config / Schema / Perf / React / Webhook / Debug / CMS)로 묶고 grep key 와 함께 나열, 정답 수정은 `dist/docs/` 로 라우팅. |
...
| 스킬 | 다루는 내용 |
| --- | --- |
| [`opt-ui-install`](./skills/opt-ui-install/) | Tailwind v4 + `OptThemeProvider` + 26단계 doctor + Surface CLI 워크플로우. |
...
| [`opt-harness-install`](./skills/opt-harness-install/) | HarnessAppShell + 반응형 Nav + 5개 레시피 중 하나로 첫 Workspace 페이지. opt-ui·opt-palette 필수, opt-datagrid·opt-editor 통합. |
...
`command/`, `references/`, `scripts/` 같은 하위 디렉터리는 v2 에서 폐지되었습니다. 장문 콘텐츠는 타깃 패키지의 `dist/docs/` 에 둡니다.
